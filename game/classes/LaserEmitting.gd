...
func _physics_process(delta):
    ...
    var results = PortalUtils.intersect_ray(space_state, global_position, global_position + (direction * 10000), [parent] + get_tree().get_nodes_in_group("transparent") + get_tree().get_nodes_in_group("fake-white"), BinaryLayers.FLOOR)
